...
def normal_quantile_mapping(var_data_obs,var_data_hist,var_data_pred):
    ...
    if (bias_corr[~np.isposinf(bias_corr)]).shape[0] > 0:
        bias_corr[np.isposinf(bias_corr)] = np.max(bias_corr[~np.isposinf(bias_corr)])
    else:
        bias_corr[np.isposinf(bias_corr)] = np.max(var_data_pred)
    if np.sum(np.isnan(bias_corr)) > 0 :
        print("Nans",np.sum(np.isnan(bias_corr)))
        print("Values ",var_data_pred[np.isnan(bias_corr)])
    elif np.sum(np.isposinf(bias_corr) )> 0 :
        print("Pos Infs",np.sum(np.isposinf(bias_corr)))
        print("CDf==1 ,mu and sig",np.sum(cdf == 1),omu,osig,np.mean(var_data_pred))
        print("Values ",var_data_pred[cdf == 1])
        print("Values ",var_data_pred[np.isposinf(bias_corr)])
        print("Nan Max", np.max(bias_corr[~np.isposinf(bias_corr)]))
    elif np.sum(np.isneginf(bias_corr)) > 0 :
        print("Neg Infs",np.sum(np.isneginf(bias_corr)))
        print("CDf==1 ,mu and sig",np.sum(cdf == 1),omu,osig)
        print("Values ",var_data_pred[cdf == 1])
        print("Values ",var_data_pred[np.isneginf(bias_corr)])
        ...
        for j in range(N2g):
            # if mask[i,j] == False:
                ...
                for day_iter in range(365):
                    va_mean = np.mean(data_obs_m_ex[day_iter:day_iter+31,:,i,j],axis= 0)
                    va_mean_trend = (moving_average(va_mean,9) - np.mean(va_mean))
                    va_mean_trend_repeated = va_mean_trend[:,np.newaxis].repeat(31,axis= 1)
                    va= (data_obs_m_ex[day_iter:day_iter+31,:,i,j] -va_mean_trend_repeated.T).flatten(order='F')   

#                     print("Tr 1" ,va_mean_trend_repeated)
#                     print("data 1" ,data_obs_m_ex[day_iter:day_iter+31,:,i,j])
#                     print("Ch 1" ,va.shape,va_mean.shape,
#                           va_mean_trend_repeated.shape,data_obs_m_ex[day_iter:day_iter+31,:,i,j].shape)


                    vb_mean = np.mean(data_gcm_train_m_ex[day_iter:day_iter+31,:,i,j],axis= 0)
                    vb_mean_trend = (moving_average(vb_mean,9) - np.mean(vb_mean))
                    vb_mean_trend_repeated = vb_mean_trend[:,np.newaxis].repeat(31,axis= 1)
                    vb=(data_gcm_train_m_ex[day_iter:day_iter+31,:,i,j]-vb_mean_trend_repeated.T).flatten(order='F')

                
                    vc=data_gcm_pred_m[day_iter,:,i,j].flatten(order='F')
                    data_trend_pred = moving_average(vc,9) - np.mean(vc)
                    data_bc_gcm[day_iter,:,i,j]=bias_correction_function(va,vb,vc-data_trend_pred)+data_trend_pred
    ...
    for i in tqdm(range(N1g)):
        for j in range(N2g):
            for month_iter in range(12):
                ...
                va= va -(moving_average(va,9) - np.mean(va))

                vb=data_gcm_train[month_iter,:,i,j].flatten()
                vb =vb - (moving_average(vb,9) - np.mean(vb))
                
                
                vc=data_gcm_pred[month_iter,:,i,j].flatten()                    

                data_trend_pred = moving_average(vc,9)  - np.mean(vc)
                data_bc_gcm[month_iter,:,i,j] = bias_correction_function(va,vb,vc-data_trend_pred).flatten() + data_trend_pred
...
def bcsd(experiment_id,data_obs_train_gridded,data_gcm_train_gridded,data_gcm_test_gridded,
    ...
    if mode ==1:
        ...
        np.save(path_out + '/detrended_BC_outputs_'+experiment_id,data_gcm_bc_pred)
    else:
        data_gcm_bc_pred=np.load(path_out + '/detrended_BC_outputs_'+experiment_id+'.npy')
    ...
    np.save(path_out + '/detrended_BCSD_outputs_'+experiment_id,data_bcsd_pred)
