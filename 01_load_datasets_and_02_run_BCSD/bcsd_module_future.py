...
def normal_quantile_mapping(var_data_obs,var_data_hist,var_data_pred):
    ...
    mufut,sigfut = scipy.stats.norm.fit(var_data_pred)
    cdf = scipy.stats.norm.cdf(var_data_pred,loc=mufut,scale=sigfut)

    omu,osig = scipy.stats.norm.fit(var_data_obs)
    bias_corr = scipy.stats.norm.ppf(cdf,loc=omu,scale=osig) + var_data_pred - scipy.stats.norm.ppf(cdf,loc=mu,scale=sig)
...
def gamma_quantile_mapping(var_data_obs,var_data_hist_train,var_data_hist_pred):
    ...
    p_zeros_hist=count_zeros/count_total
    ...
    count_zeros=(data[data<0].shape[0])
    count_total=(data.shape[0])
    p_zeros_future=count_zeros/count_total
    fitfuta,fitfutloc,fitfutscale = scipy.stats.gamma.fit(data_non_zeros,floc=0)
    
    index_zeros = (data<1)
    data[index_zeros] = 1
    cdf= p_zeros_future + (1 - p_zeros_future) * scipy.stats.gamma.cdf(data, a=fitfuta,loc=fitfutloc,scale=fitfutscale)
    cdf[index_zeros] = p_zeros_future
    ...
    for itr in range(cdf.shape[0]):
        cdfi=cdf[itr]
        point_future = var_data_hist_pred[itr]
        if cdfi<=p_zeros_imd:
            if cdfi<=p_zeros_hist:
                z1=0
                z2=(cdfi-p_zeros_hist)/(1-p_zeros_hist)

                point_o=0
                point_h=scipy.stats.gamma.ppf(z2, a=fita,loc=fitloc,scale=fitscale)

                bc_f=  point_future

                bias_corrected[itr]= point_future - point_h + point_o
            else:
                
                z1=(cdfi-p_zeros_imd)/(1-p_zeros_imd)
                z2=(cdfi-p_zeros_hist)/(1-p_zeros_hist)

                point_o=scipy.stats.gamma.ppf(z1, a=ofita,loc=ofitloc,scale=ofitscale)
                point_h=scipy.stats.gamma.ppf(z2, a=fita,loc=fitloc,scale=fitscale)
...
def empirical_quantile_mapping(var_data_obs,var_data_hist_train,var_data_hist_pred,nbins=20):
    ...
    hist_fut = np.histogram(var_data_hist_pred, bins=nbins)
    hist_dist_fut = scipy.stats.rv_histogram(hist_fut)
    cdf = hist_dist_fut.cdf(var_data_hist_pred)
    
    
    histo = np.histogram(var_data_obs, bins=nbins)
    hist_disto = scipy.stats.rv_histogram(histo)
    
    bias_corr= hist_disto.ppf(cdf)  + var_data_hist_pred - hist_dist.ppf(cdf) 
...
def bcsd(experiment_id,data_obs_train_gridded,data_gcm_train_gridded,data_gcm_test_gridded,
    ...
    if mode ==1:
        ...
        np.save(path_out + '/future_BC_outputs_'+experiment_id,data_gcm_bc_pred)
    else:
        data_gcm_bc_pred=np.load(path_out + '/future_BC_outputs_'+experiment_id+'.npy')
    ...
    np.save(path_out + '/future_BCSD_outputs_'+experiment_id,data_bcsd_pred)
