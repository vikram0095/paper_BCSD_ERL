...
def bias_corr_spatial_daily(data_obs_coarse,data_gcm_train,data_gcm_pred,dic,bias_correction_function):
    ...
    data_obs_m=data_obs_coarse.flatten(order='F').reshape((365,-1,N1g,N1g),order='F') # DOYxNxN1xN2
    data_gcm_train_m=data_gcm_train.flatten(order='F').reshape((365,-1,N1g,N1g),order='F')
    data_gcm_pred_m=data_gcm_pred.flatten(order='F').reshape((365,-1,N1g,N1g),order='F')
    del data_obs_coarse
    del data_gcm_train
    del data_gcm_pred
        ...
        for j in range(N2g):
            # if mask[i,j] == False:
                ...
                for day_iter in range(365):
                    ...
                    vc=data_gcm_pred_m[day_iter+15,:,i,j].flatten(order='F')
    ...
    for i in tqdm(range(N1g)):
        for j in range(N2g):
            for month_iter in range(12):
                ...
                data_bc_gcm[month_iter,:,i,j] = bias_correction_function(va,vb,vb).flatten()
...
def bcsd(experiment_id,data_obs_train_gridded,data_gcm_train_gridded,data_gcm_test_gridded,
    ...
    if mode ==1:
        ...
        np.save(path_out + '/BC_outputs_'+experiment_id,data_gcm_bc_pred)
    else:
        data_gcm_bc_pred=np.load(path_out + '/BC_outputs_'+experiment_id+'.npy')
    ...
    np.save(path_out + '/BCSD_outputs_'+experiment_id,data_bcsd_pred)
